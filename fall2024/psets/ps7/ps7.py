...
def sat_3_coloring(G):
    ...
    for i in range(num):
        ...
        solver.add_clause([z, z + 1, z + 2])

    for j, s in enumerate(G.edges):
        for ele in s:
            solver.add_clause([- (j+1) * 10, - ((ele+1) * 10)])
            solver.add_clause([- ((j+1) * 10 + 1), - ((ele+1) * 10 + 1)])
            solver.add_clause([- ((j+1) * 10 + 2), - ((ele+1) * 10 + 2 )])
